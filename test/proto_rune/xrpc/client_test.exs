defmodule ProtoRune.XRPC.ClientTest do
  ...
  describe "execute/1 with response: :auto" do
    test "decodes JSON when the response has no content-type header" do
      http = stub_response(%{status: 200, body: JSON.encode!(%{"handle" => "alice.bsky.social"})})

      query = Query.new("app.bsky.actor.getProfile")

      assert {:ok, %{handle: "alice.bsky.social"}} = Client.execute(query, http: http)
    end
    ...
    test "returns the raw body for a non-JSON content-type" do
      ...
      assert {:ok, %{content_type: "application/vnd.ipld.car", body: @car_bytes}} = Client.execute(query, http: http)
    end
  end
  ...
  describe "execute/1 with response: :binary" do
    test "returns the raw body even for a JSON content-type" do
      ...
      assert {:ok, %{content_type: "application/json", body: ^body}} = Client.execute(query, http: http)
    end

    test "returns a nil content_type when the response has no headers" do
      http = stub_response(%{status: 200, body: @car_bytes})
      ...
      assert {:ok, %{content_type: nil, body: @car_bytes}} = Client.execute(proc, http: http)
    end
  end
  ...
  describe "execute/1 error paths" do
    test "decodes the JSON error body even for a binary request" do
      ...
      assert {:error, %ProtoRune.XRPC.Error{}} = Client.execute(query, http: http)
    end
  end
end
