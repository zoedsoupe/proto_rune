defmodule ProtoRune.SessionManagerTest do
  ...
  describe "SessionManager" do
    test "refreshes the session through the session behaviour" do
      ...
      Application.put_env(:proto_rune, :http_stub_handler, fn :post, url, _opts ->
      ...
      {:ok, pid} = SessionManager.start_link(session: session, refresh_fraction: 0.0)
    ...
    test "stops with refresh_failed when the refresh errors" do
      ...
      Application.put_env(:proto_rune, :http_stub_handler, fn :post, _url, _opts ->
        {:ok, %{status: 500, body: "{}"}}
      end)
